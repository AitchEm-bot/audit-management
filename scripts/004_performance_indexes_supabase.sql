...
CREATE INDEX IF NOT EXISTS idx_tickets_due_date_status
...
-- Note: This partial index with NOW() may need adjustment
CREATE INDEX IF NOT EXISTS idx_tickets_created_recent
...
CREATE INDEX IF NOT EXISTS idx_tickets_report_covering
ON public.audit_tickets(created_at DESC, status, priority, department);
-- If INCLUDE is supported in your Supabase instance, use this instead:
-- CREATE INDEX IF NOT EXISTS idx_tickets_report_covering
-- ON public.audit_tickets(created_at DESC, status, priority, department)
...
CREATE INDEX IF NOT EXISTS idx_tickets_created_brin
...
CREATE INDEX IF NOT EXISTS idx_profiles_id_include
...
CREATE INDEX IF NOT EXISTS idx_comments_ticket_user
ON public.audit_comments(ticket_id, user_id, created_at DESC);

-- Optimize activity timeline queries
CREATE INDEX IF NOT EXISTS idx_activities_ticket_created
...
-- Run this query to see all indexes on audit_tickets table:
-- SELECT indexname, indexdef FROM pg_indexes WHERE tablename = 'audit_tickets' ORDER BY indexname;